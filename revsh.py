...
#
# Usage: python3 revsh.py [iface] [-p port]
####################################################################################################
...
def get_good_iface():
    """Returns the first good non-loopback interface and it's IP.

    Returns:
        The first non-loopback interface and it's IP. None if not found.
    """
...
def get_iface(prefer):
    ...
    Returns:
        The preferred network interface's name and IP.
    """
    ifaces = ifcs.interfaces()
